...
public abstract class EplKboTeamCrawler {
	...
	public EplKboTeamCrawler(TeamService teamService) {
		this.teamService = teamService;
	}

	protected void crawlTeam(String url, TeamCategory teamCategory) {
		webDriver = WebDriverConfig.createDriver();
		webDriver.get(url);
		WebDriverWait wait = new WebDriverWait(webDriver,  Duration.ofSeconds(10));
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.className("TableBody_table_body__DlwwS")));
		saveTeams(teamCategory);
		webDriver.quit();
	}

	private void saveTeams(TeamCategory teamCategory) {
		for (WebElement team : getTeamList()) {
			System.out.println(getName(team)+getLogo(team));
			// save(getName(team), getLogo(team), teamCategory);
		}
	}

	private void save(String name, String logo, TeamCategory teamCategory) {
		this.teamService.save(TeamServiceRequest.createRequest(name, logo, teamCategory));
	}
